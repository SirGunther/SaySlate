...
{
  // LD-040: a 503 on the schema request fails at once - no retry - and the error carries the
  // status and Google's own reason, with the credential redacted.
  let callCount = 0;
  const client = createClient(async () => {
    callCount += 1;
    return { ok: false, status: 503, async json() { return { error: { message: "Unavailable for key secret-key-9 right now." } }; } };
  });
  ...
  assert.equal(callCount, 1, "a 503 must not be retried");

  console.log("generateStructured LD-040 503 detail and redaction (no retry) verified.");
}

{
  // LD-039: a 429 is not retried, but it explains itself.
  let callCount = 0;
  const client = createClient(async () => {
    callCount += 1;
